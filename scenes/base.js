export class BaseScene extends Phaser.Scene {
  ...
  create(tilemapKey) {
    ...
    // this.LayerToCollide = this.map.createLayer("ItemsCollisionLayer", rpg_tileset, 0, 0);
    ...
    this.map.filterObjects("Objects", obj => {
      ...
      // FOOD
        ...
        this.foods.push(this.add.food(obj.x, obj.y, obj.properties[1].value, obj.properties[0].value))
        // Last parameters are the text to show and the direction of the text in relation to the object
      }
    });
  ...
  update(time, delta) {
    ...
    // // Hide the food
    if (this.showingFood && (moveleft || moveright || moveup || movedown)) {
      this.foods.forEach((food) => {
        if (food.activated) 
          food.playerMovement(moveleft, moveright, moveup, movedown);
      });
    }
    // Hide the bigSign
    this.bigSign.hideSignText(this.player);  // Needs to be outside the conditional in case the player goes out and immediately stops moving
  }
